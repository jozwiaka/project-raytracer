...
#define METAL 1
...
#endif
        scene.Objects.emplace_back(std::make_shared<ObjectSphere>(Vec3(0.0f, -100.5f, -1.0f), 100.0f, materialGround));
        scene.Objects.emplace_back(std::make_shared<ObjectSphere>(Vec3(0.0f, 0.0f, -1.0f), 0.5f, materialCenter));
        scene.Objects.emplace_back(std::make_shared<ObjectSphere>(Vec3(-1.0f, 0.0f, -1.0f), 0.5f, materialLeft));
        scene.Objects.emplace_back(std::make_shared<ObjectSphere>(Vec3(1.0f, 0.0f, -1.0f), 0.5f, materialRight));
    ...
    static void RandomSpheres(std::shared_ptr<Image> &img, Camera &camera, Scene &scene, Renderer &renderer, uint32_t numSamples, uint32_t maxDepth)
    {
        ...
        scene.Objects.emplace_back(std::make_shared<ObjectSphere>(Vec3(0.0f, -1000, 0.0f), 1000, groundMat));
        for (int a = -11; a < 11; a++)
        {
            for (int b = -11; b < 11; b++)
            {
                ...
                if (Math::Length(center - Vec3(4.0f, 0.2f, 0.0f)) > 0.9f)
                {
                    ...
                    if (chooseMat < 0.8f)
                    {
                        ...
                        scene.Objects.emplace_back(std::make_shared<ObjectSphere>(center, 0.2f, sphereMat));
                    }
        ...
        scene.Objects.emplace_back(std::make_shared<ObjectSphere>(Vec3(0.0f, 1.0f, 0.0f), 1.0f, material1));
        auto material2 = std::make_shared<MaterialLambertian>(Color(0.4f, 0.2f, 0.1f));
        scene.Objects.emplace_back(std::make_shared<ObjectSphere>(Vec3(-4.0f, 1.0f, 0.0f), 1.0f, material2));
        auto material3 = std::make_shared<MaterialMetal>(Color(0.7f, 0.6f, 0.5f), 0.0f);
        scene.Objects.emplace_back(std::make_shared<ObjectSphere>(Vec3(4.0f, 1.0f, 0.0f), 1.0f, material3));

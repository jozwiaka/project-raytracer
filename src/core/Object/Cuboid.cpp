...
bool Cuboid::Intersect(const Ray &ray, float &t, glm::vec3 &hitPoint, glm::vec3 &normal) const
{
    ...
    // xy
    p0 = glm::vec3(0, 0, Size.z / 2); // point on the plane
    n = glm::vec3(0, 0, 1);           // normal to the plane
    np0 = glm::dot(n, p0);
    ...
    // yz
    p0 = glm::vec3(Size.x / 2, 0, 0); // point on the plane
    n = glm::vec3(1, 0, 0);           // normal to the plane
    np0 = glm::dot(n, p0);
    t = (np0 - glm::dot(n, rayLocal.Origin)) / glm::dot(n, rayLocal.Direction);
    if (t >= 0)
    {
        ...
        if (
            Math::IsAlmostEqual(glm::dot(n, p), np0) &&
            Math::IsWithinRange(p.y, 0, Size.y / 2) &&
            Math::IsWithinRange(p.z, 0, Size.z / 2))
        {
            hitPoint = ray.Origin + t * ray.Direction;
            normal = glm::normalize((hitPoint - Center));
            return true;
    ...
    // zx
    p0 = glm::vec3(0, Size.y / 2, 0); // point on the plane
    n = glm::vec3(0, 1, 0);           // normal to the plane
    np0 = glm::dot(n, p0);
    t = (np0 - glm::dot(n, rayLocal.Origin)) / glm::dot(n, rayLocal.Direction);
    if (t >= 0)
    {
